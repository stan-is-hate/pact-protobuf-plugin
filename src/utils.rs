...
pub fn split_name(name: &str) -> (&str, Option<&str>) {
  // Technically we should only split if the name is fully qualified, ie starts with a dot `.`
  // but it doesn't matter for our use cases - message/method/service names cannot have a dot in them anyway,
  // so if the dot is there, it's fully qualified, because there are no nested packages in protobuf?
  name.rsplit_once('.')
    .map(|(package, name)| {
...
pub fn find_message_descriptor_for_type_in_vec(
  ...
  let (message_name, package) = split_name(type_name);
...
use bytes::Bytes;
  ...
  use crate::utils::{
    to_fully_qualified_name, as_hex, find_enum_value_by_name, find_nested_type, is_map_field, last_name, split_name
  };
...
use super::{build_grpc_route, find_file_descriptors, find_message_descriptor_for_type, find_method_descriptor_for_service, find_service_descriptor_for_type, parse_grpc_route, split_service_and_method};
  ...
  fn split_name_test() {
    ...
    expect!(split_name(".")).to(be_equal_to(("", Some(""))));
    expect!(split_name("test.")).to(be_equal_to(("", Some("test"))));
    expect!(split_name(".test")).to(be_equal_to(("test", Some(""))));
